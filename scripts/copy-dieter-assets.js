...
function main() {
  ...
  const compSrc = path.resolve(__dirname, '..', 'dieter', 'components');
  const compDst = path.resolve(dst, 'components');
  ...
  copyRecursiveSync(src, dst);
  // Also copy Dieter component CSS if present
  try {
    if (fs.existsSync(compSrc)) {
      copyRecursiveSync(compSrc, compDst);
    }
  } catch (_) {}
